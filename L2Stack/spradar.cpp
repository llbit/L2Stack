...
SPRadar::SPRadar(System & sys, const string &mode, const tetlist_t &tets)
	: Game(sys, mode, tets)
{
	this->mode = mode;
	this->SetUp(sys);
...
void SPRadar::TearDown()
{
}

rotsys_t *SPRadar::BuildRotSys()
{
...
int SPRadar::GetWait(wait_t wait)
{
	switch (wait) {
		case WAIT_SPEED:
			while (level >= radar_speed[speed_offset+1][0]) {
				++speed_offset;
			}
			return radar_speed[speed_offset][1];
		case WAIT_DASG:
			return 256;
		case WAIT_ARE:
		case WAIT_LARE:
...
int SPRadar::GetRule(rule_t rule)
{
	switch (rule) {
	case RULE_TLS:
...
void SPRadar::OnSpawn(System &sys, tetromino_t, bool first)
{
	int	next = level + 1;
	if (first) return;
	if (next < goal) {
...
void SPRadar::OnLock(System &sys, int lines)
{
