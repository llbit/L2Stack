...
void Archive::ReadIndex()
{
	...
	for (u32 i = 0; i < nitems; ++i) {
		...
		//std::cerr << "item: " << new_item.name << " (" << new_item.size << ")\n";
...
void Archive::WriteArchive()
{
	...
	for (itr = items.begin(); itr != items.end(); ++itr) {
		...
		if (file.fail()) {
			std::cerr << "Could not open the file: " << itr->name << "\n";
			throw "fail";
		}
	...
	if (!file.fail()) {
		...
		*data = new i8[*size];
		...
		for (it = items.begin(); it != items.end(); ++it) {
			...
			if (it->name == fn) {
				*data = new Byte[it->size];
				*size = it->size;

				stream.seekg(it->data_offset, std::ios_base::beg);
				Bytef	*code = new Byte[it->raw_size];
				stream.read((char *)code, it->raw_size);
				int result = uncompress((Bytef*)*data, (uLongf *)size, (Bytef *)code, it->raw_size);
				delete[] code;
				...
				switch (result) {
					case Z_OK:
						return;
					case Z_MEM_ERROR:
						std::cerr << "Zlib: Out of memory!\n";
						break;
					case Z_BUF_ERROR:
						std::cerr << "Zlib: Buffer too small!\n";
						break;
					case Z_DATA_ERROR:
						std::cerr << "Zlib: Corrupt input.\n";
						break;
				}
				std::cerr << "While unpacking " << fn << "\n";
